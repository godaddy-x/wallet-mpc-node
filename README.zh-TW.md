...
├── build_mobile.sh                  # gomobile → 單 ABI AAR
├── build_mobile_all.sh              # arm64 + x86_64 兩個 AAR（CI / Release）
...
| `wallet-mpc-node-arm64.aar` | Android **arm64-v8a**（實機） |
| `wallet-mpc-node-x86_64.aar` | Android **x86_64**（模擬器） |
